...
public class MatchPane extends GridPane {
    ...
    private static final Integer MV_DISP_SPACING = MC_TEAM_SPACING; //number of spaces avail for status
     ...
     *-----*/
    private static final String MV_LBL_MATCH = "   Match: ";
    ...
    private Label lblMatch, 
                  lblWinnerStatus, 
                  lblTeam1, 
                  lblTeam2;		// Match label, winner and team names
	...
	private String winnerStr = String.format("%-" + MV_DISP_SPACING + "s", 
                        	                     MV_LBL_WINNER_STATUS + 
                        	                     MV_LBL_WINNER_RESULT);
	
	
	
	
	/**
	 * Default constructor:  Places components but does not initialize them
	 */
	 ...
	 */
	public MatchPane(int number, String name1, String name2) {
		...
		// Add match header
		lblMatch = new Label(MV_LBL_MATCH + number);
		this.add(lblMatch, 
		         MV_MATCH_COL_IND, 
		         MV_MATCH_ROW_IND, 
		         MV_MATCH_COL_SPAN, 
		...
		btnSubmit.setOnAction(event -> {
			try {
				...
				winnerStr = String.format("%-" + MV_DISP_SPACING + "s", 
                                    (MV_LBL_WINNER_STATUS + 
                                    " Winner = " +
